...
router.get('/stalk/pinterest', async (req, res, next) => {
	...
	if (!username) return res.json({ Message : "Masukan Namanya"})  
	if (!apikey) return res.json(loghandler.apikey)
    fetch(encodeURI(`https://api.pinterest.com/v3/pidgets/users/${username}/pins/`)).then(response => response.json()).then(data => {
        var result = data;
             res.json({
 				result: {
...
//―――――――――――――――――――――――――――――――――――――――――― ┏  Canvas  ┓ ―――――――――――――――――――――――――――――――――――――――――― \\
router.get('/maker/levelup', async (req, res, next) => {
    var apikey = req.query.apikey
    ...
    if (listkey.includes(apikey)) {
        lol = await getBuffer(`https://api.akuari.my.id/canvas/rankup?avatar=${url}`)
        await fs.writeFileSync(__path + '/tmp/levelup.jpeg', lol)
        res.sendFile(__path + '/tmp/levelup.jpeg')
    } else {
        res.json(loghandler.apikey)
    }
})
router.get('/maker/welcome', async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Namanya"})  
	if (!grup ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Nama Grup"})  
    if (!member ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Member"})  
	if (!pp ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  PP"})  
    if (!bg ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'PanZs', message : "cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'PanZs', message : "cek kembali url image bg"}) 
...
router.get('/maker/goodbye', async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Nama"})  
	if (!grup ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Nama Grup"})  
    if (!member ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Member"})  
    if (!bg ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Bg"})  
	if (!pp) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  PP"})
	if ( !imgpp ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Link PP Yg Bener"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'PanZs', message : "Masukan Param  Link BG Yg Bener"}) 
...
router.get("/asupan/loli", async (req, res, next) => {
	...
	if (listkey.includes(apikey)) {
		var data = [
			...
			"https://storage.itsrose.my.id/asupan/loli/loli22.mp4",
      ...
      "https://storage.itsrose.my.id/asupan/loli/loli44.mp4",
...
//―――――――――――――――――――――――――――――――――――――――――― ┏  Downloader  ┓ ―――――――――――――――――――――――――――――――――――――――――― \\
router.get('/download/wattpadread', async (req, res, next) => {
          var apikey = req.query.apikey
       	var url = req.query.url
       	if(!apikey) return res.json(loghandler.noapikey)
        if(!url) return res.json({ status : false, creator : `${creator}`, message : "Masukan Parameter Url"})
        if(listkey.includes(apikey)){
alya = await fetchJson(`https://api.alyachan.my.id/api/wattpadread?url=${url}`).then((data) => {
...
router.get('/download/wattpaddetail', async (req, res, next) => {
          var apikey = req.query.apikey
       	var url = req.query.url
       	if(!apikey) return res.json(loghandler.noapikey)
        if(!url) return res.json({ status : false, creator : `${creator}`, message : "Masukan Parameter Url"})
        if(listkey.includes(apikey)){
alya = await fetchJson(`https://api.alyachan.my.id/api/wattpadget?url=${url}`).then((data) => {
  var result = data.reades
res.json({
			status: true,
			creator: `PanZs`,
			result
...
router.get('/download/igstory', async (req, res, next) => {
        ...
        if(!url) return res.json({ status : false, creator : `${creator}`, message : "Masukan Parameter Url"})
        if(listkey.includes(apikey)){
alya = await fetchJson(`https://api.alyachan.my.id/api/igs?url=${url}`).then((data) => {
  var result = data.data
res.json({
			status: true,
			creator: `PanZs`,
...
router.get('/download/spotify', async (req, res, next) => {
          var apikey = req.query.apikey
       	var url = req.query.url
       	if(!apikey) return res.json(loghandler.noapikey)
        if(!url) return res.json({ status : false, creator : `${creator}`, message : "Masukan Linknya"})
        if(listkey.includes(apikey)){
alya = await fetchJson(`https://api.alyachan.my.id/api/spotify?url=${url}`).then((data) => {
  var result = data.data
res.json({
			status: true,
			creator: `PanZs`,
...
router.get('/search/wattpad', async (req, res, next) => {
        ...
        if(listkey.includes(apikey)){
alya = await fetchJson(`https://api.alyachan.my.id/api/wattpad?q=${q}`).then((data) => {
  var result = data.data
res.json({
			status: true,
			creator: `PanZs`,
...
router.get('/search/spotify', async (req, res, next) => {
        ...
        if(!q) return res.json({ status : false, creator : `${creator}`, message : "Masukan Parameter Text"})
        if(listkey.includes(apikey)){
alya = await fetchJson(`https://api.alyachan.my.id/api/searchspotify?q=${q}`).then((data) => {
  var result = data.data
res.json({
			status: true,
			creator: `PanZs`,
